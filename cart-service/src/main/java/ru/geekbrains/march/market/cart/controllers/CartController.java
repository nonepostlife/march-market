...
public class CartController {
    ...
    public void deleteProductFromCart(@RequestHeader(required = false) String username, @PathVariable String guestCartId, @PathVariable Long productId) {
        String currentCartId = selectCartId(username, guestCartId);
        cartService.deleteProductFromCart(currentCartId, productId);
    }

    @GetMapping("/{guestCartId}/remove/{productId}")
    public void removeItemFromCart(@RequestHeader(required = false) String username, @PathVariable String guestCartId, @PathVariable Long productId) {
        String currentCartId = selectCartId(username, guestCartId);
        cartService.removeItemFromCart(currentCartId, productId);
    }

    @GetMapping("/{guestCartId}/merge")
    public void mergeGuestCartWithUserCart(@RequestHeader String username, @PathVariable String guestCartId) {
